...
function paCards(arrData,place){
  let cards= '';
...
function buttonFunction(e){
  ...
  if(searchEvent != ''){
    eventsearch = pastEvents.filter(event => (event.name.toLowerCase().includes(searchEvent.toLowerCase())));
  }
  if(categoryChecked.length > 0){
    let eventChecked = pastEvents.filter(event => categoryChecked.includes(event.category));
    eventChecked = eventChecked.forEach(event => eventsearch.push(event));
  }
  paCards(eventsearch,pastCards);
}
